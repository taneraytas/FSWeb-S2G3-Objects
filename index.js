...
function MenuElemaniOlustur(item,price,category){
	let menu = {
		isim : item,
		fiyat : price,
		kategori : category
...
const burger = {
	...
	indirim : function (type){
		if (type === 'öğretmen' || type === 'öğrenci'){
...
function SonDegerlendirmeyiAl(dizi){
	let sonElemanIndex = dizi.length-1;
	let sonEleman = dizi[sonElemanIndex];
...
function PuanaGoreDegerlendirmeAl(array,aralik) {
	let arr = []
    for (let i = 0; i < array.length; i++) {
		const element = array[i];
		if (Math.floor(element.puan) === aralik){
			arr.push(element)
		}
	}
...
function UzunDegerlendirmeleriAl(array,kelimeSayisi) {
    for (let i = 0; i < array.length; i++) {
		let element = array[i];
		if (element.geribildirim.split(" ").length>=kelimeSayisi) {
			console.log(element)
		}
	}
}
// UzunDegerlendirmeleriAl(degerlendirmeler,15)
...
function arabaYapici(kmValue) {
    const car = {
		ad : "araba1",
		km : 10, 
		sürüsCesidi : function (kmValue){
			 this.km += kmValue;
			 return console.log(this.km);
		}
	}
    return car.sürüsCesidi(kmValue)
}
arabaYapici(100);
